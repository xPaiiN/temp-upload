...
# load costs default config file
config_defaults = confuse.Configuration('diskover_{0}'.format(plugin_name), __name__)
default_config_filename = os.path.join(scriptpath_parent, 'configs_sample/diskover_costs/config.yaml')
...
try:
    ...
    if base == 10:
        ...
        basen = 1024
try:
    sizefield = config['sizefield'].get()
except confuse.NotFoundError as e:
    config_warn(e)  
    sizefield = config_defaults['sizefield'].get()
try:
    costs_list = config['costs'].get()
except confuse.NotFoundError as e:
    config_warn(e)  
    costs_list = config_defaults['costs'].get()
...
def log_setup():
    """Setup logging for diskover costs."""
    ...
    for c in costs_list:
        ...
        while res['hits']['hits'] and len(res['hits']['hits']) > 0:
            for hit in res['hits']['hits']:
                docid = hit['_id']
                # convert bytes to gb
                size_gb = hit['_source'][sizefield] / (basen * basen * basen)
                costpergb = round(cost * size_gb, 6)
                yield (docid, costpergb)
    ...
    while True:
        ...
        for res in results:
            ...
            d = {
                ...
                '_index': index,
                '_id': docid,
                'doc': {'costpergb': costpergb}
            }
            docs.append(d.copy())
            doccount += 1

        if options.verbose:
            logger.info('thread {0} started updating costs in {1} docs'.format(i, doccount))
        ...
        if options.verbose:
            logger.info('thread {0} finished updating costs for {1} docs in {2}s'.format(i, doccount, tagtime))

        del docs[:]
        costs_queue.task_done()


def index_update_costs(es, indexname):
    # Get all files from diskover index and enqueue to costs queue
    results = []
    for res in index_get_files(es, indexname):
        results.append(res)
        if len(results) >= es_chunksize:
            costs_queue.put(results[:])
            del results[:]
    costs_queue.put(results)
...
if __name__ == "__main__":
    usage = """Usage: diskover-costs.py [-h] [index]

diskover costs v{0}
Update costpergb field in docs in a diskover Elasticsearch index.""".format(version)
    parser = optparse.OptionParser(usage=usage)
    parser.add_option('-v', '--verbose', action='store_true', 
                        help='verbose logging')
    parser.add_option('--version', action='store_true',
                        help='print diskover-costs version number and exit')
    options, args = parser.parse_args()
    
    if options.version:
        print('diskover-costs v{}'.format(version))
        sys.exit(0)
    ...
    logger.info('Starting diskover costs ...')

    # print config being used
    config_filename = os.path.join(config.config_dir(), confuse.CONFIG_FILENAME)
    logger.info('Config file: {0}'.format(config_filename))
    logger.info('Config env var DISKOVER_COSTSDIR: {0}'.format(os.getenv('DISKOVER_COSTSDIR')))
    ...
    for i in range(maxthreads):
        t = Thread(daemon=True, target=costs_thread, args=(i,))
        t.start()

    logger.info('Finding and updating costs in index {0}...'.format(index))
    index_update_costs(es, index)